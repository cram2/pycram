...
from pycram.ros.ros_tools import get_parameter
from pycram.ros_utils.tf_broadcaster import TFBroadcaster
from pycram.ros_utils.viz_marker_publisher import VizMarkerPublisher
from pycrap import Apartment, Robot
...
def start_demo():
    # get params
    environment_param = get_parameter('/nbparam_environments')
    robot_param = get_parameter('/nbparam_robots')
    task_param = get_parameter('/nbparam_tasks')
...
def start_demo_local():
    # get params
    environment_param = 'apartment_without_walls'
    robot_param = 'hsrb'
    task_param = 'follow-buttons'
    ...
    robot = Object(robot_name, Robot, f"{robot_name}{extension}", pose=Pose([1, 2, 0]))
    apartment = Object(environment_param, Apartment, f"{environment_param}{extension}")
...
def demo_selecting(apartment, robot, task_param):
    ...
    if task_param == "follow":
        ...
        specialized_task = get_parameter('/nbparam_specialized_task')
