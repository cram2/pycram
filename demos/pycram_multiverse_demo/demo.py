...
robot = Object(
    "pr2", pycrap.Robot, f"pr2{extension}", pose=PoseStamped.from_list([1.3, 2, 0.01])
)
...
with simulated_robot:
    ...
    NavigateAction(
        target_locations=[PoseStamped.from_list([1.7, 2, 0])]
    ).resolve().perform()
    ...
    TransportAction(
        milk_desig, [PoseStamped.from_list([2.4, 3, 1.02])], [Arms.LEFT]
    ).resolve().perform()

    # Find and navigate to the drawer containing the spoon
    handle_desig = ObjectPart(
        names=["cabinet10_drawer1_handle"], part_of=apartment_desig.resolve()
    )
    drawer_open_loc = AccessingLocation(
        handle_desig=handle_desig.resolve(), robot_desig=robot_desig.resolve()
    ).resolve()

    NavigateAction([drawer_open_loc.pose]).resolve().perform()

    OpenAction(
        object_designator_description=handle_desig, arms=[drawer_open_loc.arm]
    ).resolve().perform()

    arm_ee = RobotDescription.current_robot_description.get_arm_chain(
        drawer_open_loc.arm
    ).get_tool_frame()
    ...
    LookAtAction(
        [apartment.get_link_pose("cabinet10_drawer1_handle")]
    ).resolve().perform()
    ...
    pick_up_loc = CostmapLocation(
        target=spoon_desig.pose,
        reachable_for=robot_desig.resolve(),
        reachable_arm=pickup_arm,
        grasp_descriptions=GraspDescription(
    ...
    PickUpAction(
        spoon_desig,
        [pickup_arm],
        GraspDescription(ApproachDirection.FRONT, VerticalAlignment.TOP, False),
    ).resolve().perform()

    ParkArmsAction(
        [Arms.LEFT if pickup_arm == Arms.LEFT else Arms.RIGHT]
    ).resolve().perform()
    ...
    placing_loc = CostmapLocation(
        target=spoon_target_pose, reachable_for=robot_desig.resolve()
    ).resolve()
...
def debug_place_spoon():
    robot.set_pose(
        PoseStamped.from_list(
            [1.66, 2.56, 0.01], [0.0, 0.0, -0.04044101807153309, 0.9991819274072855]
        )
    )
    spoon.set_pose(
        PoseStamped.from_list(
            [1.9601757566599975, 2.06718019258626, 1.0427727691273496],
            [
                0.11157527804553227,
                -0.7076243776942466,
                0.12773644958149588,
