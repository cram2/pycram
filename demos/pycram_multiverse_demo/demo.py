...
robot = Object('pr2', pycrap.Robot, f'pr2{extension}', pose=PoseSteamped.from_list([1.3, 2, 0.01]))
apartment = Object("apartment", pycrap.Apartment, f"apartment{extension}")

milk = Object("milk", pycrap.Milk, f"milk.xml", pose=PoseSteamped.from_list([2.4, 2, 1.02]),
              color=Color(1, 0, 0, 1))

spoon = Object("spoon", pycrap.Spoon, "spoon.xml", pose=PoseSteamped.from_list([2.5, 2.2, 0.85]),
               color=Color(0, 0, 1, 1))
...
with simulated_robot:
    ...
    NavigateAction(target_locations=[PoseSteamped.from_list([1.7, 2, 0])]).resolve().perform()

    LookAtAction(targets=[PoseSteamped.from_list([2.6, 2.15, 1])]).resolve().perform()
    ...
    TransportAction(milk_desig, [PoseSteamped.from_list([2.4, 3, 1.02])], [Arms.LEFT]).resolve().perform()
    ...
    spoon_target_pose = PoseSteamped.from_list([2.35, 2.6, 0.95], [0, 0, 0, 1])
...
def debug_place_spoon():
    robot.set_pose(PoseSteamped.from_list([1.66, 2.56, 0.01], [0.0, 0.0, -0.04044101807153309, 0.9991819274072855]))
    spoon.set_pose(PoseSteamped.from_list([1.9601757566599975, 2.06718019258626, 1.0427727691273496],
