...
robot = Object(
    "pr2", pycrap.Robot, f"pr2.urdf", pose=PoseStamped.from_list([1.3, 2.6, 0.01])
)
WorldStateUpdater(
    tf_topic="/tf",
    joint_state_topic="/real/pr2/joint_states",
    update_rate=timedelta(seconds=2),
    world=world,
)
...
with real_robot:
    ...
    NavigateAction(
        target_locations=[
            PoseStamped.from_list([1.4, 3.15, 0.01], quaternion_from_euler(0, 0, 3.14))
        ]
    ).resolve().perform()
