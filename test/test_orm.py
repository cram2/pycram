...
class ORMTaskTreeTestCase(test_task_tree.TaskTreeTestCase):
    ...
    def tearDown(self):
        ...
        pycram.orm.base.ProcessedMetaData.reset()
        pycram.orm.base.Base.metadata.drop_all(self.engine)
        self.session.close()
    ...
    def test_node(self):
        ...
        self.plan()
        pycram.orm.base.ProcessedMetaData().description = "Unittest"
    ...
    def test_meta_data(self):
        ...
        pycram.orm.base.ProcessedMetaData().description = "Unittest"
        pycram.task.task_tree.root.insert(self.session, )
        metadata_results = self.session.query(pycram.orm.base.ProcessedMetaData).all()
        self.assertEqual(1, len(metadata_results))

        action_results = self.session.query(pycram.orm.action_designator.Action).all()
        self.assertTrue(all([a.processed_metadata_id for a in action_results]))

        park_arms_results = self.session.query(pycram.orm.action_designator.ParkArmsAction).all()
        self.assertTrue(all([a.processed_metadata_id for a in park_arms_results]))

        object_results = self.session.query(pycram.orm.object_designator.Object).all()
        self.assertTrue(all([o.processed_metadata_id for o in object_results]))

    def test_meta_data_alternation(self):
        meta_data = pycram.orm.base.ProcessedMetaData()
        meta_data.description = "Test"
        self.plan()
        pycram.task.task_tree.root.insert(self.session, )
        metadata_result = self.session.query(pycram.orm.base.ProcessedMetaData).first()
...
class RelationshipTestCase(test_task_tree.TaskTreeTestCase):
    ...
    def test_metadata(self):
        pycram.orm.base.ProcessedMetaData().description = "MetaDataRelationshipTest"
        self.plan()
        pycram.task.task_tree.root.insert(self.session)
        result = self.session.query(pycram.orm.base.Position).all()
        print([r.processed_metadata for r in result])
        self.assertTrue(all([r.processed_metadata is not None for r in result]), msg="Failing, cause the "
    ...
    def test_task_tree_node_parents(self):
        self.plan()
        pycram.orm.base.ProcessedMetaData().description = "taskTest"
        pycram.task.task_tree.root.insert(self.session)
