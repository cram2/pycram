...
class TestActionDesignatorGrounding(BulletWorldTestCase):
    ...
    def test_navigate(self):
        description = action_designator.NavigateActionDescription([PoseSteamped.from_list([0.3, 0, 0], [0, 0, 0, 1])])
        with simulated_robot:
            self._test_validate_action_pre_perform(description, NavigationGoalNotReachedError)
            description.resolve().perform()
        self.assertEqual(description.resolve().target_location, PoseSteamped.from_list([0.3, 0, 0], [0, 0, 0, 1]))
        self.assertEqual(self.robot.get_pose(), PoseSteamped.from_list([0.3, 0, 0]))
    ...
    def test_place(self):
        object_description = object_designator.ObjectDesignatorDescription(names=["milk"])
        description = action_designator.PlaceActionDescription(object_description, [PoseSteamped.from_list([1.3, 1, 0.9],
                                                                                                [0, 0, 0, 1])],
                                                    [Arms.LEFT])
        self.assertEqual(description.resolve().object_designator.name, "milk")
        with simulated_robot:
    ...
    def test_look_at(self):
        description = action_designator.LookAtAction.description([PoseSteamped.from_list([1, 0, 1])])
        self.assertEqual(description.resolve().target, PoseSteamped.from_list([1, 0, 1]))
        with simulated_robot:
            self._test_validate_action_pre_perform(description, LookAtGoalNotReached)
            description.resolve().perform()

    def test_detect(self):
        self.kitchen.set_pose(PoseSteamped.from_list([10, 10, 0]))
        self.milk.set_pose(PoseSteamped.from_list([1.5, 0, 1.2]))
    ...
    def test_transport(self):
        ...
        description = action_designator.TransportActionDescription(object_description,
                                                        [PoseSteamped.from_list([-1.35, 0.78, 0.95],
    ...
    def test_grasping(self):
        self.milk.set_pose(PoseSteamped.from_list([-1.4, 1, 1]))
        self.robot.set_pose(PoseSteamped.from_list([-2.14, 1.06, 0]))
