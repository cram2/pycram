...
class CostmapTestCase(ApartmentWorldTestCase):
    ...
    def test_partition_into_rectangles(self):
        ocm = OccupancyCostmap(
            ...
            resolution=0.02,
            robot_view=self.robot_view,
            origin=PoseStamped.from_list([0, 0, 0], [0, 0, 0, 1], self.world.root),
            world=self.world,
        )
...
class SemanticCostmapTestCase(ApartmentWorldTestCase):
    ...
    def test_generate_map(self):
        costmap = AlgebraicSemanticCostmap(
    ...
    def test_iterate(self):
        ...
        for sample in iter(costmap):
            ...
            self.assertTrue(
                costmap.valid_area.contains([sample.position.x, sample.position.y])
            )
...
class ProbabilisticCostmapTestCase(ApartmentWorldTestCase):
    ...
    def test_visualization(self):
        fig = go.Figure(
            self.costmap.distribution.plot(), self.costmap.distribution.plotly_layout()
        )
    ...
    def test_visibility_cm(self):
        costmap = ProbabilisticCostmap(
            self.origin, size=200 * centimeter, costmap_type=VisibilityCostmap
        )
        costmap.visualize()
