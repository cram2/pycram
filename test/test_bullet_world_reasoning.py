...
class TestCaseBulletWorldReasoning(BulletWorldTestCase):
    ...
    def test_contact(self):
        self.milk.set_pose(PoseSteamped.from_list([1, 1, 1]))
        self.cereal.set_pose(PoseSteamped.from_list([1, 1, 1]))
        time.sleep(1)
    ...
    def test_occluding(self):
        self.milk.set_pose(PoseSteamped.from_list([3, 0, 1.2]))
        self.robot.set_pose(PoseStamped())
    ...
    def test_reachable(self):
        self.robot.set_pose(PoseStamped())
        time.sleep(1)
        self.assertTrue(btr.reachable(PoseSteamped.from_list([0.5, -0.5, 0.5]), self.robot,
                                      RobotDescription.current_robot_description.kinematic_chains[
                                          "right"].get_tool_frame()))
        self.assertFalse(btr.reachable(PoseSteamped.from_list([2, 2, 1]), self.robot,
                                       RobotDescription.current_robot_description.kinematic_chains[
                                           "right"].get_tool_frame()))


    def test_blocking(self):
        self.milk.set_pose(PoseSteamped.from_list([0.5, -0.5, 0.7]))
        self.robot.set_pose(PoseStamped())
        time.sleep(2)
        blocking = btr.blocking(PoseSteamped.from_list([0.5, -0.5, 0.7]), self.robot,
                                RobotDescription.current_robot_description.kinematic_chains["right"])
        self.assertTrue(blocking != [])

    def test_supporting(self):
        self.milk.set_pose(PoseSteamped.from_list([1.3, 0, 0.9]))
