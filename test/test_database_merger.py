...
class ExamplePlans():
    def __init__(self):
        ...
        self.cereal = Object("cereal", "cereal", "breakfast_cereal.stl", pose=Pose([2.5, 2.3, 1.05]), color=[0, 1, 0, 1])
    ...
    def pick_and_place_plan(self):
        with simulated_robot:
            ...
            PickUpAction(object_designator_description=self.cereal_desig, arms=[pickup_arm], grasps=["front"]).resolve().perform()
            ...
            place_stand = CostmapLocation(place_island.pose, reachable_for=self.robot_desig, reachable_arm=pickup_arm).resolve()
