...
class TestOntologyCreation(unittest.TestCase):
    def test_Performables_parsed(self):
        ...
        performable_class = next(
            filter(lambda c: c._name == "Performable", ontology.classes())
        )
    ...
    def test_Parameters_parsed(self):
        ...
        create_ontology_from_performables(output_path=tmp_file.name)
        ontology = owlready2.get_ontology(tmp_file.name).load()
        parameter_class = next(
            filter(lambda c: c._name == "Parameter", ontology.classes())
        )
    ...
    def test_Enums_parsed(self):
        ...
        ontology = owlready2.get_ontology(tmp_file.name).load()
        enum_class = next(
            filter(lambda c: c._name == "Enum", ontology.ontology.classes())
        )
    ...
    def test_information_correctly_parsed(self):
        ...
        expected_class_gripper_state_description = (
        ...
        expected_GripperState_param_description = (
        ...
        create_ontology_from_performables(
            ...
            abstract_actions_to_parse=TestOntomaticPerformable,
        )
        ontology = owlready2.get_ontology(tmp_file.name).load()

        clazz_GripperState = next(
            filter(lambda c: c._name == "GripperState", ontology.classes())
        )
        clazz_Arms = next(filter(lambda c: c._name == "Arms", ontology.classes()))
        clazz_Performable = next(
            filter(lambda c: c._name == "Performable", ontology.classes())
        )
        clazz_bool = next(filter(lambda c: c._name == "bool", ontology.classes()))
        for subclass in next(
            filter(lambda c: c._name == "Enum", ontology.classes())
        ).subclasses():
        ...
        for instance in next(
            filter(lambda c: c._name == "Arms_Value", ontology.classes())
        ).instances():
            self.assertIn(instance.name, expected_arm_value_instances)
        for instance in next(
            filter(lambda c: c._name == "GripperState_Value", ontology.classes())
        ).instances():
            self.assertIn(instance.name, expected_gripper_state_value_instances)
        self.assertIn(
            expected_class_gripper_state_description,
            str(clazz_GripperState.has_description),
        )
        self.assertIn(expected_class_arm_description, str(clazz_Arms.has_description))
        self.assertEqual(
            clazz_Performable.instances()[0].name, "TestOntomaticPerformable"
        )
        self.assertIn(
            expected_class_performable_description,
            str(clazz_Performable.instances()[0].has_description),
        )
        self.assertEqual(expected_Arms_param_name, clazz_Arms.instances()[0].name)
        self.assertIn(
            expected_Arms_param_description,
            str(clazz_Arms.instances()[0].has_description),
        )
        self.assertEqual(
            expected_GripperState_param_name, clazz_GripperState.instances()[0].name
        )
        self.assertIn(
            expected_GripperState_param_description,
            str(clazz_GripperState.instances()[0].has_description),
        )
        self.assertEqual(expected_bool_param_name, clazz_bool.instances()[0].name)
        self.assertIn(
            expected_bool_param_description,
            str(clazz_bool.instances()[0].has_description),
        )
