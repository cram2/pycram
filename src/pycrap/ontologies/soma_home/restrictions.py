...
Compartment.is_a = [DesignedComponent]

PancakeMix.is_a = [DesignedSubstance]

Tableware.is_a = [DesignedTool]
...
AffordsBeingSitOn.is_a = [Affordance, describes.some(CanBeSatOn), defines_bearer.only(SittingDestination), defines_trigger.only(And([AgentRole, classifies.only(has_disposition.some(CanSit))])), defines_task.only(Sitting)]

CanBeSatOn.is_a = [Deposition, is_described_by.some(AffordsBeingSitOn)]

SittingDestination.is_a = [Destination, classifies.only(has_disposition.some(CanBeSatOn))]

CanSit.is_a = [Capability]

AgentRole.is_a = [CausativeRole, classifies.only(Agent)]

Sitting.is_a = [AssumingPose]
...
Crockery.is_a = [DesignedContainer, Tableware, has_disposition.exactly(1, Insertion), has_disposition.exactly(1, And([Insertion, affords_trigger.only(classifies.only(Substance))]))]

BoxShape.is_a = [ShapeRegion, has_height.exactly(1, float), has_length.exactly(1, float), has_width.exactly(1, float)]

BreadKnife.is_a = [KitchenKnife]

Building.is_a = [DesignedContainer]
...
Ceiling.is_a = [RoomSurface]

RoomSurface.is_a = [Surface]

Floor.is_a = [RoomSurface]

Wall.is_a = [RoomSurface]

CeramicCooktop.is_a = [ElectricCooktop]

ElectricCooktop.is_a = [Cooktop]
...
Cooktop.is_a = [DesignedComponent]

Countertop.is_a = [DesignedComponent]
...
DesignedSpade.is_a = [DesignedComponent]

DessertFork.is_a = [Fork]
...
Dispenser.is_a = [DesignedContainer]
...
FreezerCompartment.is_a = [Compartment]
...
Hotplate.is_a = [DesignedComponent]
...
InductionCooktop.is_a = [ElectricCooktop]
...
KitchenUnit.is_a = [DesignedFurniture]
...
Oven.is_a = [Appliance, DesignedContainer, has_disposition.some(TemperingByHeating)]

TemperingByHeating.is_a = [Tempering]

ParkingArms.is_a = [AssumingArmPose]

PepperShaker.is_a = [Shaker]
...
Stove.is_a = [Appliance, has_physical_component.some(Hotplate)]
...
TableKnife.is_a = [KitchenKnife]

TableSpoon.is_a = [Spoon]

TeaSpoon.is_a = [Spoon]

Tap.is_a = [DesignedComponent]
...
TemperingByCooling.is_a = [Tempering]
...
Wardrobe.is_a = [Cupboard]

WaterBottle.is_a = [Bottle]

ThreeDPosition.is_a = [SpaceRegion, has_position_data.exactly(1, str)]

NonmanifestedSituation.is_a = [Thing]
...
has_color_value.domain = [ColorRegion, ColorRegion]
...
has_depth.domain = [ShapeRegion, ShapeRegion]
has_depth.range = [float, float]

has_shape_parameter.is_a = [DatatypeProperty, has_region_data_value]
has_shape_parameter.domain = [ShapeRegion, ShapeRegion]
has_shape_parameter.range = [Or([float, float, float, str]), Or([float, float, float, str])]
...
has_file_path.domain = [Entity, Entity]
has_file_path.range = [str, str]

has_force_value.is_a = [DatatypeProperty, has_region_data_value]
has_force_value.domain = [ForceAttribute, ForceAttribute]
has_force_value.range = [float, float]

has_friction_value.is_a = [DatatypeProperty, has_region_data_value]
has_friction_value.domain = [FrictionAttribute, FrictionAttribute]
has_friction_value.range = [float, float]

has_hsv_value.is_a = [DatatypeProperty, has_color_value]
has_hsv_value.domain = [ColorRegion, ColorRegion]
has_hsv_value.range = [str, str]

has_height.is_a = [DatatypeProperty, has_shape_parameter]
has_height.domain = [ShapeRegion, ShapeRegion]
has_height.range = [float, float]
...
has_joint_effort.domain = [JointState, JointState]
has_joint_effort.range = [float, float]

has_joint_parameter.is_a = [DatatypeProperty, has_region_data_value]
has_joint_parameter.domain = [PhysicalAttribute, PhysicalAttribute]
has_joint_parameter.range = [float, float]

has_joint_effort_limit.is_a = [DatatypeProperty, has_joint_parameter]
has_joint_effort_limit.domain = [JointLimit, JointLimit]
has_joint_effort_limit.range = [float, float]

has_joint_position.is_a = [DatatypeProperty, has_joint_parameter]
has_joint_position.domain = [JointState, JointState]
has_joint_position.range = [float, float]

has_joint_position_max.is_a = [DatatypeProperty, has_joint_parameter]
has_joint_position_max.domain = [JointLimit, JointLimit]
has_joint_position_max.range = [float, float]

has_joint_position_min.is_a = [DatatypeProperty, has_joint_parameter]
has_joint_position_min.domain = [JointLimit, JointLimit]
has_joint_position_min.range = [float, float]

has_joint_velocity.is_a = [DatatypeProperty, has_joint_parameter]
has_joint_velocity.domain = [JointState, JointState]
has_joint_velocity.range = [float, float]

has_joint_velocity_limit.is_a = [DatatypeProperty, has_joint_parameter]
has_joint_velocity_limit.domain = [JointLimit, JointLimit]
has_joint_velocity_limit.range = [float, float]

has_length.is_a = [DatatypeProperty, has_shape_parameter]
has_length.domain = [ShapeRegion, ShapeRegion]
has_length.range = [float, float]

has_mass_value.is_a = [DatatypeProperty, has_region_data_value]
has_mass_value.domain = [MassAttribute, MassAttribute]
has_mass_value.range = [float, float]
...
has_space_parameter.domain = [SpaceRegion, SpaceRegion]

has_priority.is_a = [DatatypeProperty, has_data_value]
has_priority.domain = [Task, Task]

has_rgb_value.is_a = [DatatypeProperty, has_color_value]
has_rgb_value.domain = [ColorRegion, ColorRegion]
has_rgb_value.range = [str, str]

has_radius.is_a = [DatatypeProperty, has_shape_parameter]
has_radius.domain = [ShapeRegion, ShapeRegion]
has_radius.range = [float, float]

has_reference_frame.is_a = [DatatypeProperty, has_space_parameter]
has_reference_frame.domain = [SpaceRegion, SpaceRegion]
has_reference_frame.range = [str, str]

has_shape_scale.is_a = [DatatypeProperty, has_shape_parameter]
has_shape_scale.domain = [ShapeRegion, ShapeRegion]
has_shape_scale.range = [float, float]

has_width.is_a = [DatatypeProperty, has_shape_parameter]
has_width.domain = [ShapeRegion, ShapeRegion]
has_width.range = [float, float]
...
affordance_defines.domain = [Affordance, Affordance]
affordance_defines.range = [Concept, Concept]
...
is_defined_in_affordance.domain = [Concept, Concept]
is_defined_in_affordance.range = [Affordance, Affordance]

affordance_defines_task.is_a = [ObjectProperty, affordance_defines, defines_task]
affordance_defines_task.domain = [Affordance, Affordance]
affordance_defines_task.range = [Task, Task]
...
is_task_defined_in_affordance.domain = [Task, Task]
is_task_defined_in_affordance.range = [Affordance, Affordance]

affords_bearer.is_a = [ObjectProperty, affords_concept]
affords_bearer.domain = [Disposition, Disposition]
affords_bearer.range = [Role, Role]

affords_concept.is_a = [ObjectProperty, associated_with]
affords_concept.domain = [Disposition, Disposition]
affords_concept.range = [Concept, Concept]

is_bearer_afforded_by.is_a = [ObjectProperty, is_concept_afforded_by]
is_bearer_afforded_by.domain = [Role, Role]
is_bearer_afforded_by.range = [Disposition, Disposition]
...
defines_bearer.domain = [Affordance, Affordance]
defines_bearer.range = [Role, Role]

is_concept_afforded_by.is_a = [ObjectProperty, associated_with]
is_concept_afforded_by.domain = [Concept, Concept]
is_concept_afforded_by.range = [Disposition, Disposition]

affords_performer.is_a = [ObjectProperty, affords_concept]
affords_performer.domain = [Disposition, Disposition]
affords_performer.range = [Role, Role]

is_performer_afforded_by.is_a = [ObjectProperty, is_concept_afforded_by]
is_performer_afforded_by.domain = [Role, Role]
is_performer_afforded_by.range = [Disposition, Disposition]

defines_performer.is_a = [ObjectProperty, defines_role]
defines_performer.domain = [Affordance, Affordance]
defines_performer.range = [Role, Role]

affords_setpoint.is_a = [ObjectProperty, affords_concept]
affords_setpoint.domain = [Disposition, Disposition]
affords_setpoint.range = [Setpoint, Setpoint]

is_setpoint_afforded_by.is_a = [ObjectProperty, is_concept_afforded_by]
is_setpoint_afforded_by.domain = [Setpoint, Setpoint]
is_setpoint_afforded_by.range = [Disposition, Disposition]

defines_setpoint.is_a = [ObjectProperty, defines_parameter]
defines_setpoint.domain = [Description, Description]
defines_setpoint.range = [Setpoint, Setpoint]

affords_task.is_a = [ObjectProperty, affords_concept]
affords_task.domain = [Disposition, Disposition]
affords_task.range = [Task, Task]

is_task_afforded_by.is_a = [ObjectProperty, is_concept_afforded_by]
is_task_afforded_by.domain = [Task, Task]
is_task_afforded_by.range = [Disposition, Disposition]

affords_trigger.is_a = [ObjectProperty, affords_concept]
affords_trigger.domain = [Disposition, Disposition]
affords_trigger.range = [Role, Role]

is_trigger_afforded_by.is_a = [ObjectProperty, is_concept_afforded_by]
is_trigger_afforded_by.domain = [Role, Role]
is_trigger_afforded_by.range = [Disposition, Disposition]

defines_trigger.is_a = [ObjectProperty, defines_role]
defines_trigger.domain = [Affordance, Affordance]
defines_trigger.range = [Role, Role]
...
answers.range = [Message, Message]

relates_to_another_role.is_a = [ObjectProperty, SymmetricProperty, associated_with]
relates_to_another_role.domain = [Role, Role]
relates_to_another_role.range = [Role, Role]

has_answer.is_a = [ObjectProperty, relates_to_another_role]
has_answer.domain = [Message, Message]
has_answer.range = [Answer, Answer]
...
is_reaction_to.domain = [Event, Event]
is_reaction_to.range = [Event, Event]
...
contains_object.domain = [PhysicalObject, PhysicalObject]
contains_object.range = [PhysicalObject, PhysicalObject]
...
is_inside_of.domain = [PhysicalObject, PhysicalObject]
is_inside_of.range = [PhysicalObject, PhysicalObject]

covers_object.is_a = [ObjectProperty, AsymmetricProperty, IrreflexiveProperty, interacts_with]
covers_object.domain = [PhysicalObject, PhysicalObject]
covers_object.range = [PhysicalObject, PhysicalObject]

interacts_with.is_a = [ObjectProperty, SymmetricProperty, associated_with]
interacts_with.domain = [PhysicalObject, PhysicalObject]
interacts_with.range = [PhysicalObject, PhysicalObject]

is_covered_by_object.is_a = [ObjectProperty, AsymmetricProperty, IrreflexiveProperty, interacts_with]
is_covered_by_object.domain = [PhysicalObject, PhysicalObject]
is_covered_by_object.range = [PhysicalObject, PhysicalObject]
...
is_bearer_defined_in.domain = [Role, Role]
is_bearer_defined_in.range = [Affordance, Affordance]

defines_event_type.is_a = [ObjectProperty, defines]
defines_event_type.domain = [Description, Description]
defines_event_type.range = [EventType, EventType]

is_event_type_defined_in.is_a = [ObjectProperty, is_defined_in]
is_event_type_defined_in.domain = [EventType, EventType]
is_event_type_defined_in.range = [Description, Description]

defines_input.is_a = [ObjectProperty, defines_participant]
defines_input.domain = [Description, Description]
defines_input.range = [Or([Parameter, Role]), Or([Parameter, Role])]

defines_participant.is_a = [ObjectProperty, defines]
defines_participant.domain = [Description, Description]
defines_participant.range = [Concept, Concept]

defines_output.is_a = [ObjectProperty, defines_participant]
defines_output.domain = [Description, Description]
defines_output.range = [Or([Parameter, Role]), Or([Parameter, Role])]

defines_parameter.is_a = [ObjectProperty, defines]
defines_parameter.domain = [Description, Description]
defines_parameter.range = [Parameter, Parameter]

is_parameter_defined_in.is_a = [ObjectProperty, is_defined_in]
is_parameter_defined_in.domain = [Parameter, Parameter]
is_parameter_defined_in.range = [Description, Description]

is_performer_defined_in.is_a = [ObjectProperty, is_role_defined_in]
is_performer_defined_in.domain = [Role, Role]
is_performer_defined_in.range = [Description, Description]

defines_process.is_a = [ObjectProperty, defines]
defines_process.domain = [Description, Description]
defines_process.range = [ProcessType, ProcessType]

is_process_defined_in.is_a = [ObjectProperty, is_defined_in]
is_process_defined_in.domain = [ProcessType, ProcessType]
is_process_defined_in.range = [Description, Description]

is_setpoint_defined_in.is_a = [ObjectProperty, is_parameter_defined_in]
is_setpoint_defined_in.domain = [Setpoint, Setpoint]
is_setpoint_defined_in.range = [Description, Description]

is_trigger_defined_in.is_a = [ObjectProperty, is_role_defined_in]
is_trigger_defined_in.domain = [Role, Role]
is_trigger_defined_in.range = [Affordance, Affordance]
...
describes_quality.domain = [Description, Description]
describes_quality.range = [Quality, Quality]
...
is_quality_described_by.domain = [Quality, Quality]
is_quality_described_by.range = [Description, Description]

directly_causes.is_a = [ObjectProperty, causes]
directly_causes.domain = [Event, Event]
directly_causes.range = [Event, Event]

is_direct_reaction_to.is_a = [ObjectProperty, is_reaction_to]
is_direct_reaction_to.domain = [Event, Event]
is_direct_reaction_to.range = [Event, Event]
...
encapsulates.domain = [OrderedElement, OrderedElement]
encapsulates.range = [Entity, Entity]

encodes.is_a = [ObjectProperty, SymmetricProperty, associated_with]
encodes.domain = [InformationObject, InformationObject]
encodes.range = [InformationObject, InformationObject]

executes_motion.is_a = [ObjectProperty, is_occurrence_of]
executes_motion.domain = [MotionProcess, MotionProcess]
executes_motion.range = [Motion, Motion]
...
is_executed_motion_in.domain = [Motion, Motion]
is_executed_motion_in.range = [MotionProcess, MotionProcess]
...
gives_meaning_to.domain = [System, System]
gives_meaning_to.range = [InformationObject, InformationObject]

is_given_meaning_by.is_a = [ObjectProperty, associated_with]
is_given_meaning_by.domain = [InformationObject, InformationObject]
is_given_meaning_by.range = [System, System]

has_action.is_a = [ObjectProperty, has_constituent]
has_action.domain = [Action, Action]
has_action.range = [Action, Action]
...
has_alteration_result.domain = [Action, Action]
has_alteration_result.range = [Region, Region]
...
is_alteration_result_of.domain = [Region, Region]
is_alteration_result_of.range = [Action, Action]

has_binding.is_a = [ObjectProperty, has_part]
has_binding.domain = [Description, Description]
has_binding.range = [Binding, Binding]
...
has_binding_filler.domain = [Binding, Binding]
has_binding_filler.range = [Entity, Entity]

has_binding_role.is_a = [ObjectProperty, has_part]
has_binding_role.domain = [Binding, Binding]
has_binding_role.range = [Or([Parameter, Role]), Or([Parameter, Role])]

has_child_link.is_a = [ObjectProperty, has_constituent]
has_child_link.domain = [Joint, Joint]
has_child_link.range = [PhysicalObject, PhysicalObject]

is_child_link_of.is_a = [ObjectProperty, is_constituent_of]
is_child_link_of.domain = [PhysicalObject, PhysicalObject]
is_child_link_of.range = [Joint, Joint]

has_color.is_a = [ObjectProperty, has_quality]
has_color.domain = [PhysicalObject, PhysicalObject]
has_color.range = [Color, Color]
...
is_color_of.domain = [Color, Color]
is_color_of.range = [PhysicalObject, PhysicalObject]

is_disposition_of.is_a = [ObjectProperty, is_quality_of]
is_disposition_of.domain = [Disposition, Disposition]
is_disposition_of.range = [Object, Object]

has_end_link.is_a = [ObjectProperty, has_link]
has_end_link.domain = [PhysicalObject, PhysicalObject]
has_end_link.range = [PhysicalObject, PhysicalObject]

has_link.is_a = [ObjectProperty, has_component]
has_link.domain = [PhysicalObject, PhysicalObject]
has_link.range = [PhysicalObject, PhysicalObject]

is_end_link_of.is_a = [ObjectProperty, is_link_of]
is_end_link_of.domain = [PhysicalObject, PhysicalObject]
is_end_link_of.range = [PhysicalObject, PhysicalObject]

has_execution_state.is_a = [ObjectProperty, has_region]
has_execution_state.domain = [Action, Action]
has_execution_state.range = [ExecutionStateRegion, ExecutionStateRegion]

has_feature.is_a = [ObjectProperty, has_constituent]
has_feature.domain = [PhysicalObject, PhysicalObject]
has_feature.range = [Feature, Feature]

is_feature_of.is_a = [ObjectProperty, is_constituent_of]
is_feature_of.domain = [Feature, Feature]
is_feature_of.range = [PhysicalObject, PhysicalObject]

has_first_step.is_a = [ObjectProperty, has_step]
has_first_step.domain = [Workflow, Workflow]
has_first_step.range = [Task, Task]

has_step.is_a = [ObjectProperty, defines_task]
has_step.domain = [Workflow, Workflow]
has_step.range = [Task, Task]

is_first_step_of.is_a = [ObjectProperty, is_step_of]
is_first_step_of.domain = [Task, Task]
is_first_step_of.range = [Workflow, Workflow]

has_friction_attribute.is_a = [ObjectProperty, has_region]
has_friction_attribute.domain = [PhysicalObject, PhysicalObject]
has_friction_attribute.range = [FrictionAttribute, FrictionAttribute]
...
has_initial_situation.domain = [SituationTransition, SituationTransition]
has_initial_situation.range = [Situation, Not(NonmanifestedSituation), Situation, Not(NonmanifestedSituation)]
...
is_initial_situation_of.domain = [Situation, Situation]
is_initial_situation_of.range = [SituationTransition, SituationTransition]
...
has_joint_limit.domain = [Joint, Joint]
has_joint_limit.range = [JointLimit, JointLimit]

is_joint_limit_of.is_a = [ObjectProperty, is_region_for]
is_joint_limit_of.domain = [JointLimit, JointLimit]
is_joint_limit_of.range = [Joint, Joint]

has_joint_state.is_a = [ObjectProperty, has_region]
has_joint_state.domain = [Joint, Joint]
has_joint_state.range = [JointState, JointState]

is_joint_state_of.is_a = [ObjectProperty, is_region_for]
is_joint_state_of.domain = [JointState, JointState]
is_joint_state_of.range = [Joint, Joint]
...
is_link_of.domain = [PhysicalObject, PhysicalObject]
is_link_of.range = [PhysicalObject, PhysicalObject]

has_localization.is_a = [ObjectProperty, has_quality]
has_localization.domain = [PhysicalObject, PhysicalObject]
has_localization.range = [Localization, Localization]

is_localization_of.is_a = [ObjectProperty, is_quality_of]
is_localization_of.domain = [Localization, Localization]
is_localization_of.range = [PhysicalObject, PhysicalObject]

has_mass_attribute.is_a = [ObjectProperty, has_region]
has_mass_attribute.domain = [PhysicalObject, PhysicalObject]
has_mass_attribute.range = [MassAttribute, MassAttribute]

is_mass_attribute_of.is_a = [ObjectProperty, is_region_for]
is_mass_attribute_of.domain = [MassAttribute, MassAttribute]
is_mass_attribute_of.range = [PhysicalObject, PhysicalObject]

has_net_force.is_a = [ObjectProperty, has_region]
has_net_force.domain = [PhysicalObject, PhysicalObject]
has_net_force.range = [NetForce, NetForce]

is_net_force_of.is_a = [ObjectProperty, is_region_for]
is_net_force_of.domain = [NetForce, NetForce]
is_net_force_of.range = [PhysicalObject, PhysicalObject]

has_next_step.is_a = [ObjectProperty, directly_precedes]
has_next_step.domain = [Task, Task]
has_next_step.range = [Task, Task]
...
has_previous_step.domain = [Task, Task]
has_previous_step.range = [Task, Task]
...
has_parent_link.domain = [Joint, Joint]
has_parent_link.range = [PhysicalObject, PhysicalObject]

is_parent_link_of.is_a = [ObjectProperty, is_constituent_of]
is_parent_link_of.domain = [PhysicalObject, PhysicalObject]
is_parent_link_of.range = [Joint, Joint]
...
has_predecessor.domain = [Succedence, Succedence]
has_predecessor.range = [Workflow, Workflow]

has_preference.is_a = [ObjectProperty, has_quality]
has_preference.domain = [Agent, Agent]
has_preference.range = [Preference, Preference]

is_preference_of.is_a = [ObjectProperty, is_quality_of]
is_preference_of.domain = [Preference, Preference]
is_preference_of.range = [Agent, Agent]
...
has_process_type.domain = [Role, Role]
has_process_type.range = [ProcessType, ProcessType]
...
is_process_type_of.domain = [ProcessType, ProcessType]
is_process_type_of.range = [Role, Role]

has_quale.is_a = [ObjectProperty, has_region]
has_quale.domain = [Quality, Quality]
has_quale.range = [Region, Region]

is_quale_of.is_a = [ObjectProperty, is_region_for]
is_quale_of.domain = [Region, Region]
is_quale_of.range = [Quality, Quality]

has_root_link.is_a = [ObjectProperty, has_link]
has_root_link.domain = [PhysicalObject, PhysicalObject]
has_root_link.range = [PhysicalObject, PhysicalObject]

is_root_link_of.is_a = [ObjectProperty, is_link_of]
is_root_link_of.domain = [PhysicalObject, PhysicalObject]
is_root_link_of.range = [PhysicalObject, PhysicalObject]

has_shape.is_a = [ObjectProperty, has_quality]
has_shape.domain = [PhysicalObject, PhysicalObject]
has_shape.range = [Shape, Shape]

is_shape_of.is_a = [ObjectProperty, is_quality_of]
is_shape_of.domain = [Shape, Shape]
is_shape_of.range = [PhysicalObject, PhysicalObject]

has_shape_region.is_a = [ObjectProperty, has_region]
has_shape_region.domain = [Or([Shape, PhysicalObject]), Or([Shape, PhysicalObject])]
has_shape_region.range = [ShapeRegion, ShapeRegion]

is_shape_region_of.is_a = [ObjectProperty, is_region_for]
is_shape_region_of.domain = [ShapeRegion, ShapeRegion]
is_shape_region_of.range = [Or([Shape, PhysicalObject]), Or([Shape, PhysicalObject])]

has_software_agent.is_a = [ObjectProperty, involves_agent]
has_software_agent.domain = [Event]
has_software_agent.range = [SoftwareInstance, SoftwareInstance]

has_space_region.is_a = [ObjectProperty, has_region]
has_space_region.domain = [Or([Localization, ShapeRegion, PhysicalObject]), Or([Localization, ShapeRegion, PhysicalObject])]
has_space_region.range = [SpaceRegion, SpaceRegion]

is_space_region_for.is_a = [ObjectProperty, is_region_for]
is_space_region_for.domain = [SpaceRegion, SpaceRegion]
is_space_region_for.range = [Or([Localization, PhysicalObject]), Or([Localization, PhysicalObject])]

has_state_type.is_a = [ObjectProperty, is_related_to_concept]
has_state_type.domain = [Role, Role]
has_state_type.range = [StateType, StateType]

is_state_type_of.is_a = [ObjectProperty, is_related_to_concept]
is_state_type_of.domain = [StateType, StateType]
is_state_type_of.range = [Role, Role]

has_status.is_a = [ObjectProperty, has_quality]

is_step_of.is_a = [ObjectProperty, is_task_defined_in]
is_step_of.domain = [Task, Task]
is_step_of.range = [Workflow, Workflow]

has_succedence.is_a = [ObjectProperty, has_part]
has_succedence.domain = [Workflow, Workflow]
has_succedence.range = [Succedence, Succedence]

has_successor.is_a = [ObjectProperty, has_part]
has_successor.domain = [Succedence, Succedence]
has_successor.range = [Workflow, Workflow]

has_task.is_a = [ObjectProperty, has_part]
has_task.domain = [Or([Relation, Workflow]), Or([Relation, Workflow])]
has_task.range = [Task, Task]

has_terminal_situation.is_a = [ObjectProperty, AsymmetricProperty, IrreflexiveProperty, has_terminal_state]
has_terminal_situation.domain = [SituationTransition, SituationTransition]
has_terminal_situation.range = [Situation, Not(NonmanifestedSituation), Situation, Not(NonmanifestedSituation)]
...
is_terminal_situation_of.domain = [Situation, Situation]
is_terminal_situation_of.range = [SituationTransition, SituationTransition]
...
involves_effector.domain = [Event, Event]
involves_effector.range = [PhysicalEffector, PhysicalEffector]
...
is_effector_involved_in.domain = [PhysicalEffector, PhysicalEffector]
is_effector_involved_in.range = [Event, Event]
...
is_answered_by.domain = [Event, Event]
is_answered_by.range = [Agent, Agent]
...
is_asked_by.domain = [QueryingTask, QueryingTask]
is_asked_by.range = [Agent, Agent]
...
is_created_output_of.domain = [Role, Role]
is_created_output_of.range = [Task, Task]
...
is_task_of_created_role.domain = [Task, Task]
is_task_of_created_role.range = [Role, Role]
...
is_deposit_of.domain = [PhysicalObject, PhysicalObject]
is_deposit_of.range = [PhysicalObject, PhysicalObject]

is_ontop_of.is_a = [ObjectProperty, has_location]
is_ontop_of.domain = [PhysicalObject, PhysicalObject]
is_ontop_of.range = [PhysicalObject, PhysicalObject]

is_design_for.is_a = [ObjectProperty, describes]
is_design_for.domain = [Design, Design]
is_design_for.range = [Object, Object]

is_designed_by.is_a = [ObjectProperty, is_described_by]
is_designed_by.domain = [Object, Object]
is_designed_by.range = [Design, Design]
...
is_linked_to.domain = [PhysicalObject, PhysicalObject]
is_linked_to.range = [PhysicalObject, PhysicalObject]

is_motion_description_for.is_a = [ObjectProperty, describes]
is_motion_description_for.domain = [MotionDescription, MotionDescription]
is_motion_description_for.range = [Motion, Motion]

is_moved_by_agent.is_a = [ObjectProperty, interacts_with]
is_moved_by_agent.domain = [PhysicalObject, PhysicalObject]
is_moved_by_agent.range = [Agent, Agent]

moves_object.is_a = [ObjectProperty, interacts_with]
moves_object.domain = [Agent, Agent]
moves_object.range = [PhysicalObject, PhysicalObject]

is_ordered_by.is_a = [ObjectProperty, associated_with]
is_ordered_by.domain = [OrderedElement, OrderedElement]
is_ordered_by.range = [Order, Order]

orders.is_a = [ObjectProperty, associated_with]
orders.domain = [Order, Order]
orders.range = [OrderedElement, OrderedElement]
...
is_performed_by.domain = [Action, Action]
is_performed_by.range = [Agent, Agent]

is_physically_contained_in.is_a = [ObjectProperty, is_location_of]
is_physically_contained_in.domain = [Entity, Entity]
is_physically_contained_in.range = [Entity, Entity]
...
is_replaced_by.domain = [State, State]
is_replaced_by.range = [State, State]

replaces.is_a = [ObjectProperty, after]
replaces.domain = [State, State]
replaces.range = [State, State]
...
is_supported_by.domain = [Entity, Entity]
is_supported_by.range = [Entity, Entity]
...
supports.domain = [Entity, Entity]
supports.range = [Entity, Entity]
...
is_terminated_by.domain = [Event, Event]
is_terminated_by.range = [Event, Event]

terminates.is_a = [ObjectProperty, associated_with]
terminates.domain = [Event, Event]
terminates.range = [Event, Event]
...
transitions_back.domain = [Transient, Transient]
transitions_back.range = [Object, Object]

transitions_from.is_a = [ObjectProperty, has_common_boundary]
transitions_from.domain = [Transient, Transient]
transitions_from.range = [Object, Object]

transitions_to.is_a = [ObjectProperty, has_common_boundary]
transitions_to.domain = [Transient, Transient]
transitions_to.range = [Object, Object]

has_expected_terminal_situation.is_a = [ObjectProperty, has_terminal_situation, has_postcondition]
has_expected_terminal_situation.domain = [SituationTransition, SituationTransition]
has_expected_terminal_situation.range = [Situation, Situation]
...
has_required_initial_situation.domain = [SituationTransition, SituationTransition]
has_required_initial_situation.range = [Situation, Situation]
...
prevented_by.domain = [Situation, Situation]
prevented_by.range = [Situation, Situation]
...
prevents.domain = [Situation, Situation]
prevents.range = [Situation, Situation]
