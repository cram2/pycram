...
filename = (
    get_ros_package_path("pycram") + "/resources/robots/" + "rollin_justin" + ".urdf"
)

justin_description = RobotDescription(
    "rollin_justin", "base_link", "torso2", "torso2_joint", filename
)

################################## Left Arm ##################################
left_arm = KinematicChainDescription(
    "left", "base_link", "left_arm7", justin_description.urdf_object, arm_type=Arms.LEFT
)
...
left_gripper = EndEffectorDescription(
    "left_gripper", "left_arm7", "l_gripper_tool_frame", justin_description.urdf_object
)
left_gripper.add_static_joint_states(
    GripperState.OPEN,
    {
        "left_1thumb_base_joint": 0.0,
...
torso = KinematicChainDescription(
    "torso", "torso1", "torso4", justin_description.urdf_object
)

torso.add_static_joint_states(
    TorsoState.HIGH, {"torso2_joint": 0, "torso3_joint": 0.174533, "torso4_joint": 0}
)

torso.add_static_joint_states(
    TorsoState.MID, {"torso2_joint": -0.8, "torso3_joint": 1.57, "torso4_joint": -0.77}
)

torso.add_static_joint_states(
    TorsoState.LOW,
    {"torso2_joint": -0.9, "torso3_joint": 2.33874, "torso4_joint": -1.57},
)
...
camera = CameraDescription(
    "dummy_camera", "head2", 1.27, 1.85, 0.99483, 0.75049, [1, 0, 0]
)
