...
if multiverse_resources is not None:
    urdf_filename, mjcf_filename = get_robot_urdf_and_mjcf_file_paths(
        data.name, data.relative_dir, multiverse_resources=multiverse_resources
    )

if mjcf_filename is None or urdf_filename is None:
    logger.info(
        f"Could not initialize {data.name} description as Multiverse resources path not found."
    )
else:
    robot_description = create_manipulator_description(
        data, urdf_filename, mjcf_filename
    )
