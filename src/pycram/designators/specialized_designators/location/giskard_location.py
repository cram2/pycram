...
from ....external_interfaces.giskard import (
...
class GiskardLocation(CostmapLocation):
    ...
    def __iter__(self) -> CostmapLocation.Location:
        ...
        if self.visible_for:
            raise ValueError(
                "GiskardLocation does not support the visible_for parameter"
            )
        ...
        near_costmap = OccupancyCostmap(
            0.35, False, 200, 0.02, target_map
        ) + GaussianCostmap(200, 15, 0.02, target_map)
        for maybe_pose in PoseGenerator(near_costmap, 200):
            for chain in manipulator_descs:
                projection_joint_goal(
                    chain.get_static_joint_states(StaticJointState.Park),
                    allow_collisions=True,
                )

                trajectory = projection_cartesian_goal_with_approach(
                    maybe_pose,
                    target_map,
                    chain.tool_frame,
                    "map",
                ...
                orientation = list(
                    quaternion_from_euler(
                        0, 0, last_joint_states["brumbrum_yaw"], axes="sxyz"
                    )
                )
                pose = PoseStamped.from_list(
                    [
                        last_joint_states["brumbrum_x"],
                        last_joint_states["brumbrum_y"],
                        0,
                    ],
                    orientation,
                )
                ...
                prospection_robot = (
                    World.current_world.get_prospection_object_for_object(World.robot)
                )

                with UseProspectionWorld():
                    prospection_robot.set_multiple_joint_positions(robot_joint_states)
                    prospection_robot.set_pose(pose)
                    gripper_pose = prospection_robot.get_link_pose(
                        chain.get_tool_frame()
                    )

                    if (
                        gripper_pose.position.euclidean_distance(target_map.position)
                        <= 0.02
                    ):
