...
from pycram.datastructures.enums import GripperState, VerticalAlignment, DetectionState, Arms, ApproachDirection, DetectionTechnique, TorsoState, TaskStatus, AxisIdentifier
...
class ActionDescriptionDAO(Base, DataAccessObject[pycram.robot_plans.actions.base.ActionDescription]):
    ...
    robot_position_id: Mapped[int] = mapped_column(ForeignKey('PoseStampedDAO.id', use_alter=True), nullable=True)

    robot_position: Mapped[PoseStampedDAO] = relationship('PoseStampedDAO', uselist=False, foreign_keys=[robot_position_id], post_update=True)
