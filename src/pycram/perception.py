...
from semantic_digital_twin.world_description.world_entity import SemanticAnnotation, Region, KinematicStructureEntity, \
    Body
...
class PerceptionQuery:
    ...
    def from_world(self) -> List[Body]:
        result = []
        sem_instances = self.world.get_semantic_annotations_by_type(self.semantic_annotation)
        bodies = []
        for sem_instance in sem_instances:
            bodies.extend(sem_instance.bodies)

        region_bodies = list(filter(None, [body if self.region.contains(body.global_pose.to_position()) else None for body in bodies]))

        robot_camera = list(filter(None, [cam if isinstance(cam, Camera) else None for cam in self.robot.sensors]))[0]
